...
(*
  match l with
    | nil => []
    | x :: xs => match partition test xs with
                   | (p,n) => if test x
                              then (x :: p, n)
                              else (p, x :: n)
                 end
...
when transforming a list into something else
...
Proof.
  unfold fold_map; induction l; simpl; intros; congruence.
Qed.
